...
<nav class="navbar">
  <div class="navbar-inner">
    ...
    <div class="navbar-actions">
      ...
      <% if (isAdmin) { %><span class="badge badge-violet">ADMIN</span><% } %>
...
<div class="page-wrap">
  <a href="${pageContext.request.contextPath}<%= isAdmin ? "/admin/dashboard" : "/dashboard/home" %>"
     class="btn btn-ghost btn-sm" style="margin-bottom:20px;">&#8592; Back</a>
  ...
  <div class="page-header">
    ...
    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-top:8px;">
      ...
      <span style="font-size:14px;color:var(--slate);">&#127979; <%= election.getInstitutionName() %></span>
      <span style="font-size:14px;color:var(--slate);">Total votes: <strong><%= totalVotes %></strong></span>
    </div>
    <% } %>
  </div>
  ...
  <div class="card" style="max-width:680px;">
    ...
    <div class="card-subtitle">Ranked by vote count.</div>
      ...
      <div class="result-item">
        <div class="result-header">
          <span class="result-name">
            ...
            <% } else { %>
              <span style="width:32px;height:32px;border-radius:50%;background:var(--violet);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;flex-shrink:0;"><%= c.getUserName().charAt(0) %></span>
            <% } %>
            <%= c.getUserName() %>
            <% if (first && totalVotes > 0) { %>
              <span class="winner-badge"><%= election != null && election.getStatus() == Election.Status.CLOSED ? "&#127942; Winner" : "Leading" %></span>
            <% } %>
          </span>
        ...
        <div class="result-bar-track">
          <div class="result-bar-fill"
               style="width:<%= totalVotes > 0 ? (c.getVoteCount() * 100 / totalVotes) : 0 %>%;"></div>
        </div>
        <% if (c.getManifesto() != null && !c.getManifesto().isBlank()) { %>
          <p style="font-size:13px;color:var(--slate);margin-top:2px;"><%= c.getManifesto() %></p>
        <% } %>
      </div>
      <% first = false; } %>
    </div>
