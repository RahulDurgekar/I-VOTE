...
<div class="page-wrap">
  <a href="${pageContext.request.contextPath}/dashboard/home" class="btn btn-ghost btn-sm" style="margin-bottom:20px;">&#8592; Back</a>
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;">
      <div style="display:flex;flex-direction:column;gap:6px;">
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
          <h1 style="font-size:24px;font-weight:700;color:var(--ink);"><%= election.getTitle() %></h1>
          <span class="badge badge-<%= election.getStatus() %>"><%= election.getStatus() %></span>
        </div>
        <p style="font-size:14px;color:var(--slate);"><%= election.getDescription() != null ? election.getDescription() : "" %></p>
        <div style="display:flex;gap:20px;flex-wrap:wrap;font-size:13px;color:var(--steel);margin-top:4px;">
  ...
  <% } else if ("already_voted".equals(request.getParameter("error")) || "phone_voted".equals(request.getParameter("error"))) { %>
  ...
  <% } else if ("not_active".equals(request.getParameter("error"))) { %>
    <div class="alert alert-error">&#9888; This election is not currently active.</div>
  ...
  <% } else if ("already_candidate".equals(request.getParameter("error"))) { %>
    <div class="alert alert-info">&#8505; You are already registered as a candidate.</div>
  <% } else if ("candidate".equals(request.getParameter("joined"))) { %>
    <div class="alert alert-success">&#10003; You are now registered as a candidate!</div>
  <% } %>

  <%-- TWO COLUMN LAYOUT --%>
  <div style="display:flex;gap:24px;flex-wrap:wrap;align-items:flex-start;">

    <%-- LEFT: VOTE + CANDIDATE REGISTER --%>
    <div style="flex:1;min-width:300px;display:flex;flex-direction:column;gap:20px;">

      <%-- VOTE PANEL --%>
      <div class="card">
        <div class="card-title">Cast Your Vote</div>
        <div class="card-subtitle">Select a candidate and submit. You can only vote once.</div>

        <% if (voted) { %>
          <div class="alert alert-info" style="margin-bottom:0;">&#10003; You have already voted in this election.</div>
        <% } else if (election.getStatus() == Election.Status.UPCOMING) { %>
          <div class="alert alert-warn" style="margin-bottom:0;">&#9200; Voting hasn't started yet.</div>
        <% } else if (election.getStatus() == Election.Status.CLOSED) { %>
          <div class="alert alert-warn" style="margin-bottom:0;">&#128274; This election is closed. Voting is no longer available.</div>
        <% } else if (candidates == null || candidates.isEmpty()) { %>
          <div class="alert alert-info" style="margin-bottom:0;">No candidates have registered yet.</div>
        <% } else { %>
          <form method="post" action="${pageContext.request.contextPath}/dashboard/election" id="voteForm">
            ...
            <div class="candidates-grid">
              ...
              <div class="candidate-card" onclick="selectCandidate(<%= c.getId() %>, this)">
                ...
                <% } else { %>
                  <div class="candidate-photo-placeholder"><%= c.getUserName().charAt(0) %></div>
                <% } %>
                <div class="candidate-name"><%= c.getUserName() %></div>
                <div class="candidate-manifesto"><%= c.getManifesto() != null ? c.getManifesto() : "" %></div>
                <span class="selected-mark">&#10003; Selected</span>
              </div>
              <% } %>
            </div>
            <button type="submit" id="voteBtn" disabled
                    class="btn btn-primary" style="width:100%;justify-content:center;margin-top:20px;">
              Submit Vote
            </button>
          </form>
        <% } %>
      ...
      <% if (Boolean.TRUE.equals(isCandidate)) { %>
        <div class="card">
          ...
          <div class="card-subtitle">You are participating in this election. Good luck!</div>
        </div>
      <% } else if (election.getStatus() == Election.Status.UPCOMING) { %>
        <div class="card">
          <div class="card-title">Stand as Candidate</div>
          <div class="card-subtitle">Register before the election starts with your manifesto.</div>
          <button class="btn btn-outline-violet" onclick="document.getElementById('candidateModal').classList.add('active')">
    ...
    <div class="card" style="flex:1;min-width:280px;">
      <div class="card-title">
        <% if (election.getStatus() == Election.Status.CLOSED) { %>Final Results<% } else { %>Live Results &#128202;<% } %>
      </div>
      <div class="card-subtitle">Total votes cast: <strong><%= totalVotes %></strong></div>

      <% if (results == null || results.isEmpty()) { %>
        <p style="font-size:14px;color:var(--slate);">No votes yet.</p>
      <% } else { %>
        ...
        <div class="result-item">
          <div class="result-header">
            <span class="result-name">
              ...
              <% } else { %>
                <span style="width:28px;height:28px;border-radius:50%;background:var(--violet);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;"><%= c.getUserName().charAt(0) %></span>
              <% } %>
              <%= c.getUserName() %>
              <% if (first && totalVotes > 0) { %>
                <span class="winner-badge"><%= election.getStatus() == Election.Status.CLOSED ? "&#127942; Winner" : "Leading" %></span>
              <% } %>
            </span>
            <span class="result-votes">
              <%= c.getVoteCount() %> vote<%= c.getVoteCount() != 1 ? "s" : "" %>
              <% if (totalVotes > 0) { %> (<%= String.format("%.0f", c.getVoteCount() * 100.0 / totalVotes) %>%)<% } %>
            </span>
          </div>
          <div class="result-bar-track">
            <div class="result-bar-fill" style="width:<%= totalVotes > 0 ? (c.getVoteCount() * 100 / totalVotes) : 0 %>%;"></div>
          </div>
        </div>
        <% first = false; } %>
      </div>
...
<div class="modal-overlay" id="candidateModal" onclick="if(event.target===this)this.classList.remove('active')">
  <div class="modal">
    ...
    <form method="post" action="${pageContext.request.contextPath}/dashboard/election"
      ...
      <div class="form-group">
        ...
        <p class="form-hint">Max 2MB. JPEG or PNG recommended.</p>
      </div>
      <div class="form-group">
        <label class="form-label">Your Manifesto *</label>
        <textarea name="manifesto" class="form-control" rows="4"
                  placeholder="Describe your goals and plans..." required></textarea>
      </div>
      <div class="modal-actions">
        <button type="button" class="btn btn-ghost"
                onclick="document.getElementById('candidateModal').classList.remove('active')">Cancel</button>
